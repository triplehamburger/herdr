...
By default the sidebar is split into two panels: Spaces above, Agents below, with a draggable divider between them. Every agent appears in the lower panel labelled with the space it belongs to.

Set `sidebar_mode = "unified"` to collapse both into one list instead. The Agents panel is removed, the divider goes away, the Spaces list takes the full sidebar height, and each space's agents render as indented child rows beneath it:

```toml
[ui]
sidebar_mode = "unified"
```

Each space that owns at least one agent gets an expand/collapse toggle at the end of its row. Click `▾` to collapse a space and `▸` to expand it again. The choice is remembered per client, alongside worktree group collapse, so a narrow phone attach and a wide desktop attach keep their own layout.

A collapsed space still reports the states that need you. Its row carries a count for each of blocked, done, and working, using the same indicator style as `status_indicators`, so collapsing a space never hides the fact that an agent finished or is waiting on input.

Nested agent rows use `ui.sidebar.agents.rows`, so `rows_by_agent` overrides apply in both modes. While that setting is left at its default, unified mode substitutes a nested-friendly layout that identifies the agent rather than repeating the space name on every child row. Setting `rows` explicitly always wins:
...
[ui.sidebar.agents]
rows = [["state_icon", "terminal_title_stripped"], ["agent", "tab"]]
```

Worktree groups keep their own collapse toggle. When a space is both a worktree parent and has agents, the group toggle sits just inside the space toggle.
